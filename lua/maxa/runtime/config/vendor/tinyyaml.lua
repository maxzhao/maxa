-- filepath: lua/maxa/runtime/config/vendor/tinyyaml.lua
-- -------------------------------------------------------------------------
-- VENDORED THIRD-PARTY MODULE — do not edit the parser body.
-- TinyYaml (pure-Lua YAML-subset parser), vendored verbatim from
--   https://github.com/peposso/lua-tinyyaml  (commit d280b04, 2020-08-09)
-- TinyYaml is the Lua/Neovim ecosystem's de-facto minimal YAML parser and is used
-- here to decode `.maxa/runtime.yaml`, replacing a previous hand-written parser.
--
-- MIT License
-- Copyright (c) 2017 peposso
-- (full MIT text lives in the upstream repo; see LICENSE above.)
--
-- Surface: `require('maxa.runtime.config.vendor.tinyyaml').parse(source) -> table`.
--   * parse throws (via error/table) on malformed input — wrap in pcall for the
--     fail-closed decode contract (see ../../yaml.lua).
--   * `null`/`~` scalars decode to an empty table, not `nil`.
-- -------------------------------------------------------------------------
-------------------------------------------------------------------------------
