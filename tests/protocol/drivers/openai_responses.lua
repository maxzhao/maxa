-- filepath: tests/protocol/drivers/openai_responses.lua
--- OpenAI Responses fixture driver (phase-1 W6).
---
...
--- runner's `protocol.get_adapter("openai_responses")` resolves after dofile.
---
--- Fixture flow (protocol-fixture-contract.md §"OpenAI Responses"):
---   1. adapter:setup(provider_options) -> params
---   2. adapter:build_request(params, {messages, tools}) vs request.expected_body
---   3. response feed:
---        streamed      -> sse parser feeds response.chunks (SSE frame text with
---                         `event:` lines), adapter:parse_stream per frame
---                         (one at a time)
...
---        - tool calls with decoded arguments (expected_tool_calls)
...
---   - id "openai-responses/strict-nested-schema": additionally asserts that
---     vim.json.encode(body) serializes the empty object properties as `{}`
---     and never as `[]`.
...
local M = {}
M.name = "driver.openai_responses"
...
---@param adapter table registered openai_responses adapter
...
function M.run_fixture(fixture, adapter, runner)
  ...
  -- strict-nested-schema: empty object properties must serialize as {} not [].
  if data.id == "openai-responses/strict-nested-schema" then
    local ok, enc = pcall(vim.json.encode, body)
    runner.expect(ok and type(enc) == "string", "strict schema: body must encode to JSON", failures)
    if ok and type(enc) == "string" then
      runner.expect(
        enc:find("[]", 1, true) == nil,
        "strict schema: empty object must not serialize as []",
        failures
      )
      runner.expect(
        enc:find("{}", 1, true) ~= nil,
        "strict schema: empty object must serialize as {}",
        failures
      )
    end
  end
  ...
  -- 4d. tool calls (decoded arguments)
